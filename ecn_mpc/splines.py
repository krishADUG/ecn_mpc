...
class Splines:
    ...
    def spline_from(self, t0, x0, T):
        ...
        start = self.path[idx]
        dx = np.sqrt(self.path[idx].dist_sq(x,y))
        # if dx < 1e-3:
        #     idx += 1
        #     start = self.path[idx]
        ...
        v = self.path[idx-1].relvel_to(start)
        ...
        if False:
            print(steps)
            print(xy[:,0].shape)
            x_sp = make_smoothing_spline(steps,xy[:,0], 0.1)
            y_sp = make_smoothing_spline(steps,xy[:,1], 0.1)

            def sp(t):
                return [x_sp(t), y_sp(t)]
        else:
            try:
        ...
        def ref(t, include_u = False):
            ret = xy[-1] if t-t0 > steps[-1] else sp(t-t0)
